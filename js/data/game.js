const game = {
    ...
    achievements: [
        new Achievement("Alpha", "'A journey of a thousand miles begins with a single step' - Chinese proverb", "α", () => (game.layers[0] && game.layers[0].resource && game.layers[0].resource.gte(1))),
        new Achievement("Easy alpha", "Get your first generator", 'α<sub>1</sub>', () => (game.layers[0] && game.layers[0].generators[0] && game.layers[0].generators[0].bought && game.layers[0].generators[0].bought.gt(0))),
        new Achievement("Easier alpha", "Get your second generator", 'α<sub>2</sub>', () => (game.layers[0] && game.layers[0].generators[1] && game.layers[0].generators[1].bought && game.layers[0].generators[1].bought.gt(0))),
        new Achievement("Easiest alpha", "Get your fifth generator", 'α<sub>5</sub>', () => (game.layers[0] && game.layers[0].generators[4] && game.layers[0].generators[4].bought && game.layers[0].generators[4].bought.gt(0))),
        new Achievement("Upgrade", "Get an alpha upgrade", '↑<sub>α<sub>', () => (game.layers[0] && game.layers[0].upgrades[0] && game.layers[0].upgrades[0].level && game.layers[0].upgrades[0].level.gt(0))),
        new Achievement("Upgradier", "Prestige to get a β", "β", () => game.highestLayer >= 2),
        new Achievement("Empowered", "Get β power", 'β<sub>1</sub>', () => (game.layers[1] && game.layers[1].generators[0] && game.layers[1].generators[0].bought && game.layers[1].generators[0].bought.gt(0))),
        new Achievement("Upgradiest", "Prestige to get a γ", "γ", () => game.highestLayer >= 3),
        new Achievement("Out of RAM?", "Make layer α non-volatile", '<img src="images/save.svg" alt="S">', () => (game.volatility && game.volatility.layerVolatility && game.volatility.layerVolatility.level && game.volatility.layerVolatility.level.gt(0))),
        new Achievement("Challenging", "Beat your first challenge", 'γ<sub>C</sub>', () => (game.layers[2] && game.layers[2].challenges[0] && game.layers[2].challenges[0].level && game.layers[2].challenges[0].level > 0)),
        new Achievement("Aleph-0", "Start gaining aleph", "ℵ", () => (game.alephLayer && game.alephLayer.isUnlocked())),
        new Achievement("Aleph-1", "Have 1e75 aleph", 'ℵ<sub>1</sub>', () => (game.alephLayer && game.alephLayer.aleph && game.alephLayer.aleph.gte("1e75"))),
        new Achievement("Aleph-2", "Have 1e200 aleph", 'ℵ<sub>2</sub>', () => (game.alephLayer && game.alephLayer.aleph && game.alephLayer.aleph.gte("1e200"))),
        new Achievement("Aleph-3", "Have 1.8e308 aleph", 'ℵ<sub>3</sub>', () => (game.alephLayer && game.alephLayer.aleph && game.alephLayer.aleph.gte("1.8e308"))),
        new Achievement("Stacking up", "Do a restack and restart your progress", PrestigeLayer.getNameForLayer(9), () => (game.restackLayer && game.restackLayer.timesReset && game.restackLayer.timesReset > 0)),
